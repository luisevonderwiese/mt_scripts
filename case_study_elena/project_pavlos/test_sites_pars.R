...
treeNJ  <- optim.parsimony(treeRA, phy)
#plot(treeNJ)
...
optimizeSetNJ <- function(data, cog.tree=cog.tree, init.n=ncol(data), max.tries=100000, max.fails=max.tries, minimum.sites=20, maximum.sites=400, old.keep=0.9, proposals=NULL){
    ...
    for(i in 1:rand.n){
        ...
        treeRA <- random.addition(samplephy)
    ...
    dir.create(file.path("pars_results/"), showWarnings = FALSE)
    ...
    for(i in 1:max.tries){
        ...
        if( (i==1) || (i %% 10000 == 0)){
            ...
            write.table(attrNames, file=paste("pars_results/pars_selected_features_step", i, ".txt", sep=""), quote=F, col.names=F, row.names=F)
            write.table(curRf, file=paste("pars_results/pars_current_accuracy_step", i, ".txt", sep=""), quote=F, col.names=F, row.names=F)
            samplephy = as.phyDat(datamat, type='USER', levels=c("0","1"), ambiguity="?", names=rownames(data), n=nrow(data), return.index=TRUE)
            ...
            d <- treedist(tr, cog.tree, check.labels =TRUE)
            write.phyDat(x=samplephy, file=paste("pars_results/pars_current_opt_data_large_step", i, ".nex", sep=""), format="nexus")
            write.table(new.indexes, file=paste("pars_results/pars_current_opt_indexes_large_step", i, ".csv", sep=""), sep=",", col.names=F, row.names=F, quote=F)
            write.tree(tr, file=paste("pars_results/pars_current_opt_tree_large_step", i, ".nwk", sep=""))
        }
        print(i)
        newdatamat = datamat
