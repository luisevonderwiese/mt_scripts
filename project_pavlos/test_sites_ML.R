...
system("mkdir raxml")
write.phyDat(x=phy, file="raxml/current_alignment.phy", format="phylip")
system("./raxml-ng --search1 --msa raxml/current_alignment.phy --seed 2 --threads 2 --model BIN+G  --prefix raxml/")
treeNJ=read.tree("raxml/.raxml.bestTree")
system("rm -rf raxml")
#plot(treeNJ)
...
optimizeSetNJ <- function(data, cog.tree=cog.tree, init.n=ncol(data), max.tries=100000, max.fails=max.tries, minimum.sites=20, maximum.sites=400, old.keep=0.9, proposals=NULL){
    ...
    for(i in 1:max.tries){
        ...
        if( (i==1) || (i %% 10000 == 0)){
            ...
            write.tree(tr, file=paste("ml_results/ml_current_opt_tree_large_step", i, ".nwk", sep=""))
        }
        print(i)
        newdatamat = datamat
...
system("mkdir raxml")
write.phyDat(x=samplephy, file="raxml/current_alignment.phy", format="phylip")
system("./raxml-ng --search1 --msa raxml/current_alignment.phy --seed 2 --threads 2 --model BIN+G  --prefix raxml/")       tr=read.tree("raxml/.raxml.bestTree")
system("rm -rf raxml")
